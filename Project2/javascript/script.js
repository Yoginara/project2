...
document.addEventListener('DOMContentLoaded', function () {
  // Ambil elemen input kode booking
  const inputKodeBooking = document.getElementById('inputBooking');

  // Generate kode booking secara otomatis (misalnya, angka acak 8 digit)
  const generateRandomCode = () => {
    return Math.floor(10000000 + Math.random() * 90000000);
  };

  // Isi nilai input kode booking saat dokumen selesai dimuat
  inputKodeBooking.value = generateRandomCode();

  // Mendisable kemampuan edit pada input kode booking
  inputKodeBooking.setAttribute('readonly', 'true');
